...
class TypeScriptLangUtil (LangUtil):
    ...
    def isGenericTypeBracket(self, tokens, i):
        if tokens[i] == "<":
            ...
            while level > 0 and i < len(tokens):
                ...
                if tokens[i] == ">":
                    level -= 1
                elif not re.match(TypeScriptLangUtil.genericTypeRegex, tokens[i]):
                    break
                i += 1
            return level == 0
        elif tokens[i] == ">":
    ...
    def hasSpaceBetween(self, tokens, nextI):
        ...
        if left == "--" or right == "--":
            return False
        if right == "<" or right == ">":
            return not self.isGenericTypeBracket(self, tokens, nextI)
        if left == "<" or left == ">":
            return not self.isGenericTypeBracket(self, tokens, nextI - 1)
